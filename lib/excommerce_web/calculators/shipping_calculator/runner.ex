defmodule ExcommerceWeb.ShippingCalculator.Runner do
  use GenServer
    ...
    proc_list =
      Enum.flat_map(state.shipping_methods, fn(method) ->
        Enum.map(state.order.shipment_units, fn(shipment_unit) ->
          spawn_monitor(fn ->
            send(current, Tuple.append(ExcommerceWeb.ShippingCalculator.calculate_shipping_cost(method, shipment_unit), self()))
          end)
        end)
      end)
