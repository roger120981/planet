defmodule ExcommerceWeb.Workflow.AuthorizePayment do
  alias Ecto.Multi
  ...
  defp process_payment(_changes, repo, order_changeset) do
    case order_changeset.valid? do
      true ->
        ...
        payment_method        = Excommerce.Query.PaymentMethod.get!(repo, payment_method_id) |> Map.get(:name)
        payment_method_params = params["payment_method"] || params[:payment_method] || %{}

        case ExcommerceWeb.Gateway.authorize_payment(order_changeset.data, payment_method, payment_method_params) do
