defmodule ExcommerceWeb.Workflow.CancelLineItemFullfillment do
  ...
  def steps(repo, line_item) do
    changeset = Excommerce.Orders.LineItem.fullfillment_changeset(line_item, %{fullfilled: false})
    Multi.new()
    ...
    |> Multi.append(ExcommerceWeb.Workflow.MoveStockBackFromLineItem.steps(line_item.variant, line_item.quantity))
    |> Multi.append(ExcommerceWeb.Workflow.SettleAdjustmentAndProductPaymentForOrder.steps(repo, line_item.order))
  ...
  defp cancel_order_if_no_fullfilled_line_items(_changes, repo, order) do
    if !Excommerce.Query.Order.can_be_fullfilled?(repo, order) do
      Excommerce.Command.Order.mark_as_cancelled(repo, order)
    else
      {:ok, order}
    end
  end
