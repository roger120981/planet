defmodule ExcommerceWeb.Admin.CheckoutController do
  ...
  def next(conn, %{"order" => order_params}) do
    ...
    case CheckoutManager.next(Repo, order, order_params) do
      ...
      {:ok, %Excommerce.Orders.Order{state: "payment"} = updated_order} ->
        confirmed_order = Excommerce.Query.Order.get!(Repo, updated_order.id)
  ...
  def back(conn, _params) do
    order = Excommerce.Query.Order.get!(Repo, conn.params["order_id"])
  ...
  def go_back_to_cart_if_empty(conn, _params) do
    order = Excommerce.Query.Order.get!(Repo, conn.params["order_id"]) |> Repo.preload([:line_items])
    if Excommerce.Orders.Order.cart_empty? order do
      conn
