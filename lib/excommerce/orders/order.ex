defmodule Excommerce.Orders.Order do
  ...
  alias Excommerce.Orders.{LineItem, Order, OrderBillingAddress, OrderShippingAddress}
  alias Excommerce.Shipments.{Shipping, ShipmentUnit}
  ...
  schema "orders" do
    ...
    has_many :shipment_units, ShipmentUnit
    ...
    has_one  :order_billing_address, OrderBillingAddress
    has_one  :billing_address, through: [:order_billing_address, :address]

    has_one  :order_shipping_address, OrderShippingAddress
  ...
  def changeset(struct, _params), do: struct
  ...
  def cancellation_changeset(struct) do
    struct
    |> cast(%{order_state: "cancelled"}, ~w(order_state))
  end

  def address_changeset(order, attrs \\ %{}) do
    order
    |> cast(attrs, [:same_as_billing, :state])
  ...
  def payment_changeset(order, attrs \\ %{}) do
    order
    |> cast(payment_params(order, attrs), @required_fields ++ @optional_fields)
    |> validate_required(@required_fields)
    |> cast_assoc(:payment, required: true, with: &Payment.applicable_payment_changeset/2)
  end
  ...
  def user_cart_changeset(order, attrs \\ %{}) do
    order
    |> cast(attrs, [:state, :user_id])
    |> validate_required([:state, :user_id])
  end

  def cart_update_changeset(order, attrs \\ %{}) do
    order
    |> cast(attrs, [])
    |> validate_required([])
    |> cast_assoc(:line_items, with: &LineItem.direct_quantity_update_changeset/2)
  end

  def link_to_user_changeset(order, attrs \\ %{}) do
    order
    |> cast(attrs, [:user_id])
  ...
  def settlement_changeset(order, attrs) do
    order
    |> cast(attrs, [:total, :product_total, :confirmation_status])
    |> validate_required([:total, :product_total, :confirmation_status])
  end
  ...
  def shipping_changeset(order, attrs \\ %{}) do
    ...
    |> cast_assoc(:shipment_units, required: true, with: &ShipmentUnit.create_shipment_changeset/2)
  end
  ...
  def tax_changeset(order, attrs \\ %{}) do
    order
    |> cast(attrs, [:tax_confirm, :state])
    |> validate_required([:tax_confirm, :state])
    |> validate_tax_confirmed
  end

  def transaction_id_changeset(order, attrs) do
    order
    |> cast(attrs, ~w()a)
    |> cast_assoc(:payment, with: &Payment.transaction_id_changeset/2)
  end
  ...
  def confirmation_changeset(order, attrs \\ %{}) do
    order
    |> cast(attrs, [:confirm, :state])
    |> validate_required([:confirm, :state])
    |> validate_order_confirmed
  end

  defp validate_order_confirmed(order) do
    confirmed = get_field(order, :confirm)
    if confirmed do
      order
    else
      add_error(order, :confirm, "Please confirm to finalise the order")
    end
  end

  defp validate_tax_confirmed(order) do
    confirmed = get_field(order, :tax_confirm)
    if confirmed do
      order
    else
      add_error(order, :tax_confirm, "Please confirm to proceed")
    end
  end
  ...
  def check_line_items_for_availability(order) do
    Enum.reduce(order.line_items, {true, [], [], []}, fn
      (line_item, {_status, out_of_stock, discontinued, insufficient_stock} = acc) ->
        ...
        case Variant.availability_status(variant, requested_quantity) do
          :discontinued ->
